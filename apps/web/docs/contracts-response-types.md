...
```ts
type Member = {
  user?: { id?: string; username?: string | null; avatarUrl?: string | null; wallets?: Wallet[] };
};
```
...
**Note:** this nested `{ user: {...} }` shape is _not_ what `GET /conversations/:id/members` returns — that route flattens the member/user fields into one object and adds `primaryWalletAddress`. See [DRIFT-5](#drift-5-get-conversationsidmembers-response-shape-has-no-matching-frontend-type) — no frontend type or caller for that flattened shape currently exists in `apps/web`.
