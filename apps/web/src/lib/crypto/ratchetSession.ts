/**
 * IndexedDB-backed Double Ratchet session store.
 *
 * NOTE: this layer deliberately rejects out-of-order messages
 * ("Out-of-order ratchet messages are not supported" in decryptDm). The
 * skipped-message-key handling required by #360 lives in `doubleRatchet.ts`;
 * these two have not been reconciled yet, and neither is wired into the app.
 */
