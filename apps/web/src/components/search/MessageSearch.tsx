...
export function MessageSearch({
  ...
  const { query, setQuery, hits, total, loading, error, clear } = useLocalSearch({
    conversationId,
  });
    ...
    <div className="flex flex-col gap-3 w-full">
      ...
      {showInfo && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5 text-xs text-amber-200/80 leading-relaxed">
          <strong className="text-amber-200">E2EE search — local only.</strong> Because messages are
          end-to-end encrypted, the server can&apos;t search ciphertext (#124). Search runs entirely
          in your browser, over messages this device has decrypted and cached in IndexedDB (#185).
          It works offline, runs in a Web Worker, and never leaves your device.
          <br />
          <span className="opacity-80">
            Only synced / decrypted history is searchable. New devices start with an empty index
            until history syncs.
          </span>
        </div>
      )}
